...
The old name collided with Claude Code's built-in /design (Claude Design's
canvas tool), which won the collision and left the generated skill
uninvokable — it never appeared in the skill listing at all.
...
4. Clean up retired files.
   ...
   **Everything else is a rename or a fold-in, where the replacement already exists — ask first:**
   ...
   - `.claude/skills/design/` — renamed to `design-doc/`; carry any customizations into the new directory before removing the old one. Don't leave both: the stale copy still loses the name race with Claude Code's built-in `/design`, so the user is left with a skill that never runs.
