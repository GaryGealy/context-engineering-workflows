...
RENAMED skills:
  /design → /design-doc — the old name collided with Claude Code's built-in
    /design skill (Claude Design's canvas tool), which won the name and left the
    generated skill uninvokable. The old skill directory will be removed.
...
4. Clean up retired files, **asking first**:
   ...
   - `.claude/skills/design/` — renamed to `design-doc/`; remove the old directory once the new one is written. Leaving both in place is worse than either alone: the stale copy still loses the name race with the built-in `/design`, so the user sees a skill that never runs.
5. Ask whether they also run this workflow in VS Code Copilot chat.
   - **If yes** — installs generated by 4.2.0 carry `model:` and `effort:` in every skill and agent, which hangs Copilot chat on invocation and needs a VS Code restart to clear. Say so plainly: this is the bug they may have been hitting. Regeneration alone is not enough to trust here — run the strip over the whole tree from the project root and check it exits clean:
